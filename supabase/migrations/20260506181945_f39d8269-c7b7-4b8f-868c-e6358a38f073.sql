...
-- 1 share = current_price × 100 cents. Pays 100 KSh if correct.
-- After each trade, price moves linearly by k × signed_qty,
-- clamped to [floor, ceiling]. Multi-outcome renormalizes.
-- LMSR functions left in place but no longer called by these RPCs.
-- ============================================================

-- Settings
INSERT INTO public.system_settings (key, value)
VALUES ('price_floor', '0.05'::jsonb)
ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = now();

INSERT INTO public.system_settings (key, value)
VALUES ('price_ceiling', '0.95'::jsonb)
ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = now();

INSERT INTO public.system_settings (key, value)
VALUES ('linear_impact_k', '0.0008'::jsonb)
ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = now();

-- ---------- simulate_lmsr_trade (LINEAR) ----------
...
BEGIN
  ...
  -- Linear impact (BUY pushes price up, SELL pushes down)
  _delta := _k * _quantity;
  IF _side = 'BUY' THEN
  ...
  -- Cost is FLAT: shares × current price (no curve, no average over a curve)
  _gross_cents := round(_price_before * 100 * _quantity)::bigint;
  IF _side = 'BUY' THEN
...
-- ---------- execute_lmsr_trade_binary (LINEAR) ----------
...
BEGIN
  ...
  -- Cost = shares × current price (the rule)
  _gross_cents := round(_price_before * 100 * _quantity)::bigint;
  IF _side = 'BUY' THEN
    _fee_cents := round(_gross_cents * _fee_bps / 10000.0)::bigint;
  ...
  -- Update price linearly, clamp, renormalize the pair
  _delta := _k * _quantity;
  IF _side = 'BUY' THEN
    _price_after := LEAST(_ceil, _price_before + _delta);
  ELSE
...
-- ---------- execute_lmsr_trade_multi (LINEAR + renormalize) ----------
...
BEGIN
  ...
  -- Renormalize the OTHER outcomes so prices sum to ~1
  ...
  -- Snapshot price history for all outcomes
